 ...
 */
public class WatchLaterFragment extends AbstractVideoFragment {
...
//        this.user    = getArguments().getString("user");
    ...
    private void getVideos(){
        if(!ListUtils.isEmptyList(videos.get(YoutubeHelper.WATCH_LATER_VIDEO_TYPE))) {
            videoListAdapter.addVideoList(videos.get(YoutubeHelper.WATCH_HISTORY_VIDEO_TYPE));
        }
        else{
            new GetYoutubeVideosTask(youtube,this).execute(YoutubeHelper.WATCH_LATER_VIDEO_TYPE);
        }
    }
    ...
    public void onListItemClick(ListView listView, View view, int position, long id) {
        ...
        this.mCallbacks.onItemSelected(Long.toString(id), YoutubeHelper.WATCH_LATER_VIDEO_TYPE);
    }
}
